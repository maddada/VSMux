...
const vscodeState = vi.hoisted(() => ({
  ...
  workspaceRoot: "/workspace/agent-tiler",
}));
...
describe("sidebar command preferences", () => {
  ...
  beforeEach(() => {
    ...
    vscodeState.workspaceRoot = "/workspace/agent-tiler";
    temporaryRoots = [];
  });
  ...
  test("should migrate legacy workspace commands into shared worktree storage", async () => {
    ...
    const mainContext = createMockContext(globalStorage);
    const worktreeContext = createMockContext(globalStorage);
    await mainContext.workspaceState.update("VSmux.sidebarCommands", [
      {
        actionType: "terminal",
        closeTerminalOnExit: false,
        command: "pnpm build",
  ...
  async function createGitWorktreeFixture(): Promise<{
    ...
    const root = await mkdtemp(join(tmpdir(), "vsmux-sidebar-commands-"));
    temporaryRoots.push(root);
    const mainRoot = join(root, "agent-tiler");
    const worktreeRoot = join(root, "agent-tiler-xterm-non-persistent");
    const worktreeGitDir = join(mainRoot, ".git", "worktrees", "agent-tiler-xterm-non-persistent");
