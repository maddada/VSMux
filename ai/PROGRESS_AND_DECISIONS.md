...
Close out the planned MVP phases from
`mvp-plan.md` and keep
the implementation/docs aligned with the validated runtime behavior.
...
- replaced the sample Agent Canvas X extension with an
  `agentCanvasX.openCanvasPanel` command and a reusable `WebviewPanel` host
...
- added a VS Code setting, `agentCanvasX.uiScale`, that controls the scale of
...
- replaced the old workspace-details dialog with a settings dialog that edits
  `agentCanvasX.uiScale` directly and lowered the default UI zoom to 0.60
...
- added an `agentCanvasX.terminalFontFamily` setting and defaulted terminal
...
- added untrusted-workspace shell gating so new terminal spawns and restarts
  require an explicit one-time approval before Agent Canvas X launches shells in
...
- let the webview settings dialog update `agentCanvasX.uiScale` directly so the
