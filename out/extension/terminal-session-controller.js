...
exports.TerminalSessionController = void 0;
const node_crypto_1 = require("node:crypto");
const promises_1 = require("node:fs/promises");
const os = require("node:os");
const path = require("node:path");
const vscode = require("vscode");
const terminal_host_client_1 = require("./terminal-host-client");
...
class TerminalSessionController {
    options;
    hasApprovedUntrustedShells = vscode.workspace.isTrusted;
    client;
    outputFlushTimer;
    ...
    knownSnapshotTileIds = new Set();
    workspaceId;
    constructor(options) {
        this.options = options;
        this.client = new terminal_host_client_1.TerminalHostClient({
            daemonScriptPath: path.join(options.context.extensionUri.fsPath, "out", "extension", "terminal-host-daemon.js"),
            storagePath: options.context.globalStorageUri.fsPath,
        });
        this.workspaceId = getWorkspaceId();
        this.persistedSessionRoot = path.join(options.context.globalStorageUri.fsPath, "terminal-host-daemon", "terminal-history", this.workspaceId);
    ...
    async reconcileSnapshot(snapshot) {
        this.reconcileChain = this.reconcileChain.then(async () => {
            ...
            const knownTileIds = new Set([...this.sessions.values()]
    ...
    async resetWorkspace() {
        this.reconcileChain = this.reconcileChain.then(async () => {
            ...
            try {
                await this.client.ensureConnected();
                liveSessions = (await this.client.listSessions()).filter((session) => session.workspaceId === this.workspaceId);
            }
            catch {
                liveSessions = [];
            }
            ...
            await (0, promises_1.rm)(this.persistedSessionRoot, {
                force: true,
                recursive: true,
            });
        });
    ...
    async deletePersistedSession(tileId) {
        await (0, promises_1.rm)(path.join(this.persistedSessionRoot, tileId), {
            force: true,
            recursive: true,
        });
    }
    async loadPersistedSession(tileId) {
        ...
        try {
            const [history, rawMetadata] = await Promise.all([
                (0, promises_1.readFile)(historyPath, "utf8").catch(() => ""),
    ...
    async ensureShellSpawnAllowed() {
        if (vscode.workspace.isTrusted || this.hasApprovedUntrustedShells) {
            this.hasApprovedUntrustedShells = true;
            return true;
        }
        const approval = await vscode.window.showWarningMessage("Agent Canvas X is about to start a shell in an untrusted workspace.", {
            detail: "Shell sessions can run commands against files in this workspace. Trust the workspace or explicitly allow shell access to continue.",
            modal: true,
        }, "Allow Shell Access");
        if (!approval) {
            return false;
...
exports.TerminalSessionController = TerminalSessionController;
function estimateTerminalColumns(tileWidth) {
    return Math.max(20, Math.floor((tileWidth - 32) / 8.5));
}
function estimateTerminalRows(tileHeight) {
    return Math.max(8, Math.floor((tileHeight - 72) / 18));
}
function createStartingSession(tile, options) {
    return {
...
function createDisconnectedSession(tile, options) {
    return {
...
function createBlockedSession(tile, options) {
    return {
        ...createStartingSession(tile, options),
...
//# sourceMappingURL=terminal-session-controller.js.map